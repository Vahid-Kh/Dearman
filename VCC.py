...
for refrig in range(len(R)):
    ...
    dt_approach = 3  # Min Temp difference at watercooled heat exchanger
    ...
    dt_superheat_ig = 6  # DT of superheat in (k)
    ...
    while dq >= toll_dq:
        ...
        RTSR2[5] = TDN(RTSR2_C[4].dp(dp_evap), PSI('H', 'Q', 1, 'P', RTSR2[4].p , r), 0, 0, 0, r)
        ...
        cop_r_2 = (RTSR2[5].h-RTSR2[4].h)*(1-q_req_2)/((RTSR2[8].h-RTSR2[7].h)*(1-q_req_2)+(RTSR2[10].h-RTSR2[9].h))
        ...
        RTS2R2[5] = TDN(RTS2R2[4].p, PSI('H', 'Q', 1, 'P', RTS2R2[4].p, r), 0, 0, 0, r)  # gas from reciever outlet
        ...
        cop_2r_2c = (RTS2R2[7].h - RTS2R2[6].h) * (1 - q_2r_2c_ht)* (1 - q_2r_2c_lt) / ((RTS2R2[11].h - RTS2R2[10].h) * (1 - q_2r_2c_lt) + (RTS2R2[13].h - RTS2R2[12].h))
        ...
        cop_2r_2c_i = (RTS2R2I[7].h - RTS2R2I[6].h) * (1 - q_2r_2c_ht_i) * (1 - q_2r_2c_lt_i) / (
        ...
        RTS2R2IM[5] = TDN(RTS2R2IM[4].p, PSI('H', 'Q', 1, 'P', RTS2R2IM[4].p , r), 0, 0, 0,
                          r)  # gas from reciever outlet
        ...
        cop_2r_2c_i_m = (RTS2R2IM[7].h - RTS2R2IM[6].h) * (1 - q_2r_2c_ht_i_m) * (1 - q_2r_2c_lt_i_m) / (
    ...
    p_drop_exh = 0.5 * (HXTS[4].d+exhaust.d)*(1-(1/exhaust_bore_ratio_ig)**4)*((m_dot_hx/HXTS[4].d)/(((180 - theta_c_open) / 360)*0.75* d_bore_exh**2 *mt.pi/4))**2 +0.5 * (exhaust.d)*(1-(1/exhaust_bore_ratio_ig)**4)*((m_dot_c/exhaust.d)/(((180 - theta_c_open) / 360)*0.75* d_bore_exh**2 *mt.pi/4))**2
...
fig, ax = plt.subplots()
rect0 = ax.bar(ind - width / 3, rects0, width / 4, color=(1, 0.5+.5*(cop_c[0]-min(cop_l))/(max(cop_l)-min(cop_l)), 0.3), label='1 stage - simple')
rect1 = ax.bar(ind - width / 6, rects1, width / 4, color=(1, 0.5+.5*(cop_c_r[0]-min(cop_l))/(max(cop_l)-min(cop_l)), .3 ), label='1 stage - with receiver')
rect2 = ax.bar(ind , rects2, width / 4,            color=(1, 0.5+.5*(cop_r_2_l[0]-min(cop_l))/(max(cop_l)-min(cop_l)), 0.3), label='2 stage + rec ')
rect3 = ax.bar(ind + width / 6, rects3, width / 4, color=(1, 0.5+.5*(cop_2r_2c_l[0]-min(cop_l))/(max(cop_l)-min(cop_l)), 0.3), label='2 stage + 2 rec ')
rect4 = ax.bar(ind + width / 3, rects4, width / 4, color=(1, 0.5+.5*(cop_2r_2c_i_l[0]-min(cop_l))/(max(cop_l)-min(cop_l)), 0.3), label='2 stage + 2 rec + 1 int ')
rect5 = ax.bar(ind + width / 2, rects5, width / 5, color=(1, 0.5+.5*(cop_2r_2c_i_m_l[0]-min(cop_l))/(max(cop_l)-min(cop_l)), 0.3), label='2 stage + 2 rec + 1 int + mod ')

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('COP(Coeficient of performance)')
ax.set_xlabel('Various refrigerant in with and without receiver configuration')
ax.set_title('COP changes of various refrigerant with and without receiver configuration' + ' @ T_evap =  '+ str(t_evap))
ax.set_xticks(ind)
