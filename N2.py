...
R = ['R404A', 'R22', 'n-Propane', 'CO2']
C = ['Nitrogen', 'Air', 'Methane', 'Hydrogen']
HX = ['INCOMP::MEG[0.6]', 'INCOMP::AEG[0.6]']

l ,  ss,  qq,  ww = [], [], [], []
s_tot, s_sum, m_dot = [], [], []
cop_c, cop_c_r, cop_r_2_l, cop_2r_2c_l, cop_2r_2c_i_l, cop_2r_2c_i_m_l = [], [], [], [], [], []
cryo = 2
for refrig in range(len(R)):
    ...
    while dq >= toll_dq:
        ...
        """Energy balance on superheated gas from LT compressor and gas directly coming from reciever"""
        RTS2R2IM[9] = TDN(RTS2R2IM[8].p, (RTS2R2IM[5].h * q_2r_2c_lt_i_m + RTS2R2IM[8].h * (1 - q_2r_2c_lt_i_m)*(1-q_2r_2c_ht_i_m)+ RTS2R2IM[2].h * q_2r_2c_ht_i_m), 0, 0, 0, r)
        ...
        _____________________________________________M_Dot calculation________________________________________________
        ___________________________________________________________________________________________________________"""
        ...
        """_________________________________________Matrix solution________________________________________________"""
        ...
        """---------------------------------------------------------------------------------------------------------"""
        print('Exergyyyyyyyyyyyy   :   ' + str((CTS[4].ex() - CTS[0].ex()) * m_dot_c))
        """_____________________________________Cooling effect______________________________________________________"""
    ...
    _____________________________________________Performance calculation_____________________________________________
    ______________________________________________________________________________________________________________"""
    ...
    if refrig == 0:
        ...
        def objective(x):
            ...
            v_c_min = (x[5] * (1 - mt.cos(x[1] / 180 * mt.pi)) + x[4] * (
                        1 - mt.sqrt(1 - (x[5] / x[4]) ** 2 * mt.sin(x[1] / 180 * mt.pi) ** 2)) + x[3]) * mt.pi * x[
                          2] ** 2 / 4

            v_c_max = (x[5] * (1 - mt.cos(theta_exh_open_ig / 180 * mt.pi)) + x[4] * (
                        1 - mt.sqrt(1 - (x[5] / x[4]) ** 2 * mt.sin(theta_exh_open_ig / 180 * mt.pi) ** 2)) + x[
                           3]) * mt.pi * x[2] ** 2 / 4
        ...
        def cont2(x):  # INEQ
            ...
            gam = exp_c_in.cp() / exp_c_in.cv()
            p_star = exp_c_in.p * (2 / (gam + 1)) ** ((gam) / (gam - 1))
            m_dot_lim = c_disch * (d_bore_intake ** 2) / 4 * mt.pi * mt.sqrt(
                gam * exp_c_in.d * exp_c_in.p * ((2 / (gam + 1) ** ((gam + 1) / (gam - 1)))))
        ...
        print('Optimized design objective function value :',
              objective([sol.x[0], sol.x[1], sol.x[2], sol.x[3], sol.x[4], sol.x[5], sol.x[6]]))
        print(sol)
        end = time.time()
        ...
        print('x[3]: ', 'Dead volume on top :                ', sol.x[3], '+',
              m_dot_hx / HXTS[2].d / mt.pi / sol.x[0] / (sol.x[2] ** 2) * 4, ' For HXF')
    ...
    print("Mass flow rate for Cryogen in [kg/s] is :     " + str(m_dot_c) + c)
    print("Mass flow rate for Refrigerant in [kg/s] is : " + str(m_dot_r) + r)
    print("Mass flow rate for HX fluid in [kg/s] is :    " + str(m_dot_hx) + hx)
    ...
    plt.legend(['1 Stage','1 Stage + 1 Reciever','2 stage + 1 Reciever ', '2 stage + 2 Reciever ', '2 stage + 2 Reciever + 1 intercooler'])
...
fig, ax = plt.subplots()
rect0 = ax.bar(ind - width / 3, rects0, width / 4, color=(1, ((cop_c[0]-min(cop_c))/(max(cop_c)-min(cop_c))), 0.1), label='1 stage - simple')
rect1 = ax.bar(ind - width / 6, rects1, width / 4, color=(1, ((cop_c_r[0]-min(cop_c_r))/(max(cop_c_r)-min(cop_c_r))), 0.2), label='1 stage - with receiver')
rect2 = ax.bar(ind , rects2, width / 4,            color=(1, ((cop_r_2_l[0]-min(cop_r_2_l))/(max(cop_r_2_l)-min(cop_r_2_l))), 0.3), label='2 stage + rec ')
rect3 = ax.bar(ind + width / 6, rects3, width / 4, color=(1, ((cop_2r_2c_l[0]-min(cop_2r_2c_l))/(max(cop_2r_2c_l)-min(cop_2r_2c_l))), 0.4), label='2 stage + 2 rec ')
rect4 = ax.bar(ind + width / 3, rects4, width / 4, color=(1, 1, 0.5), label='2 stage + 2 rec + 1 int ')
# rect5 = ax.bar(ind + width / 2, rects5, width / 5, color=(1, ((cop_2r_2c_i_m_l[0]-min(cop_2r_2c_i_m_l))/(max(cop_2r_2c_i_m_l)-min(cop_2r_2c_i_m_l))), 0.6), label='2 stage + 2 rec + 1 int + mod ')
...
for a in range(len(sizes0)):
    col0.append((1, 1-(sizes0[a]/sum(sizes0)/0.7), a/20))
