...
t_evap_in = 243.15   # Evaporator temperature (T_Min) - 30 C
t_l = []
m_l = []
f_l = []
cop_o_l = []
...
for refrig in range(len(R)):
    ...
    for temp in range(num_temp):
        t_evap = t_evap_in + temp
        t_l.append(-30+temp)
        # t_l.append(t_evap)
        print(-30+temp, t_evap)

        """____FLUIDS____ : """
        r = R[refrig]  # Refrigerant in the refrigeration cycle
        c = C[2]  # Cryogenic fluid used for cooling (Nitrogen in this case)
    ...
    plt.title('Off-design condition of various evaporator temperature for ' + str(R[refrig]))
