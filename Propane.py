...
for cryo in range(len(C)):
    ...
    """____FLUIDS____ : """
    r = R[0]  # Refrigerant in the refrigeration cycle
    ...
    dt_subcool_ig = - 4  # DT of sub-cooling in (k) (with minus sign)
    ...
    while dq >= toll_dq:
        ...
        if TDN(0, 0, t_exp_ig, 0, CTS[1].d, c).p > p_lim_exp:
            ...
            CTS[2] = TDN(0, 0, t_exp_ig, 0, CTS[1].d,
                         c)  # Tempereture of cryogen rises to near ambient at near isochoric process

        CTS_C[2] = CTS[2].__copy__()
        exp_c_out = CTS[2]

        CTS[3] = TDN(p_exp_min, 0, CTS_C[2].t, 0, 0,
    ...
    # TEST OF ISENTROPIC WORK
    # one = TDN(p1, 0, t_exp_ig, 0, 0, c)
    # two = TDN(p2, 0, 0, one.s, 0, c)
    # gamma = (one.cp()/one.cv() + two.cp()/two.cv())/2
    # w_isentropic = one.p/(one.d**(gamma))   *   (((1/two.d)**(1-gamma)-(1/one.d)**(1-gamma))/(1-gamma))
    # print("eeeeeeeeeeeeeee" +  str(w_isentropic))
    # print(one.prop())
    # print(two.prop())
    ...
    _______________________________________________________________________________________________________________"""
    ex = (q_hx_c * m_dot_c + q_evap_r * m_dot_r/cop) / m_dot_c
    print(ex / (CTS[0].ex()))
    ...
    s_tot_exp_exh = (exp_amb_c.s - p2) * m_dot_c
    ...
    """
    s_hx_c_exp = m_dot_hx * (hx_in_exp.h - hx_out_exp.h) * (
                1 / mlog(exp_exh_c.t, exp_c_in.t) - 1 / mlog(hx_in_exp.t, hx_out_exp.t))
    s_tot_hx_c_exp = m_dot_hx * (hx_in_exp.s - hx_out_exp.s)

    """------------------------------------------------------------------------------------------------------------"""

    """______________________________________ Refrigeration cycle ________________________________________________"""
    ...
    print("COP is :  ____ " + str(cop)+ "____")
    ...
    print("RPM     :   " + str(RPM))
    print("RPM_f   :   " + str(f*60))
    print("W_exp_required based on isothermal work : " + str(w_exp*m_dot_c))
    print("W_exp_real based on U-S, initial design : " + str(w_exp_exact_ig*eta_exp_ig))
    print("W_exp_real based on U-S, Optimal design : " + str(w_exp_real_TDN*eta_exp_ig))

    print("Max work that can be extracted from the flow : " + str(w_exp_max))
...
ax.set_title('Mass flow rate in [kg / s] for various cryogens ')
ax.set_xticks(ind)
ax.set_xticklabels(('Cryogen', 'Refrigerant', 'HX fluid'))
ax.legend()
...
print(CTS[0].ex(), CTS[4].ex())
print(CTS[0].ex() , q_hx_c ,q_exp_c*(1-t0/t_evap),-w_exp)

print((CTS[0].ex() + q_hx_c * (1 - t0 / mlog(t_evap,CTS[0].t))+(q_exp_c)*(1-t0/(mlog(t_evap,t_amb)))-w_exp)/CTS[0].ex())
print((CTS[0].ex() + q_hx_c * (1 - t0 / t_evap)+(q_exp_c)*(1-t0/t_evap)-w_exp)/CTS[0].ex())


"""Positive effects"""
ex = (q_hx_c*m_dot_c +q_evap_r*m_dot_r)/m_dot_c
print(ex/(CTS[0].ex()))
