...
for refrig in range(len(R)):
    ...
    print()
    for cool in range(num_coolreq):

        """----------- Cooling required ----------"""
        cool_req = (cool+2)*1e3  # Cooling capacity required from VCC amd cryogenic HX  (W)
        coolreq_l.append((cool+2)*1e3)
        print(cool_req)
        """------------------------------------"""
    ...
    p_l[:] = [x/1e5 for x in p_l]

    p1, = host.plot(coolreq_l, m_l_r[refrig], "b*", label= R[refrig] +" Cryogen mass flow rate [Kg/s]")
    p2, = par1.plot(coolreq_l, f_l, "r+", label=R[refrig] +           " Frequency of piston expander [1/s]")
    p3, = par2.plot(coolreq_l, p_l, "gd", label=R[refrig] +           " Maximum pressure in piston expander [bar]")

    host.set_xlim(0, max(coolreq_l))
    host.set_ylim(min(min(m_l_r))*0.7, max(max(m_l_r))*1.3)
    par1.set_ylim(min(f_l)*0.8, max(f_l)*1.2)
    par2.set_ylim(min(p_l)*0.99, max(p_l)*1.01)

    host.set_xlabel("Cooling requirement variation [Watts]")
    host.set_ylabel(" Cryogen mass flow rate [Kg/s]")
    par1.set_ylabel(" Frequency of piston expander [1/s]")
    par2.set_ylabel(" Maximum pressure in piston expander [bar]")
    ...
    plt.title('Off-design condition of cooling requirment for' + str(R[refrig]))
